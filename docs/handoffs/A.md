...
남은 것은 실제 CV의 동일 투구·릴리스 이전·좌표 검증과 제구/기여 품질, 당시 교체 후보 명단 및 별도 이닝 평가기, 식별 가능한 정책 효과와 최종 확인이다. **사용자 후속 일정: 실제 CV 인수는 약 일주일 뒤이므로 당장 작업에서 제외한다. 다음 한 가지는 A2의 비영상 선행연구 비교 사양을 실행 가능한 공통 기준선으로 구체화하는 것이다.** 자료가 없으면 unavailable을 유지하고 S2 학습 확대는 자동 진행하지 않는다.
