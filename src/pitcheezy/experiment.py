...
class Experiment:
    ...
    def stage_ope(self) -> dict:
        ...
        for name, pol in menu.items():
            ...
            tt = key.merge(DR.traj_dr_terms(ev, sid, rho_pitch, has_pitch, q_e, V_e, r_pa=r_all), on=["game_pk", "at_bat_number"], how="left").fillna(0.0)
            dr_plain, dr_wdr = tt["dr_plain"].to_numpy(), tt["dr_wdr"].to_numpy()
