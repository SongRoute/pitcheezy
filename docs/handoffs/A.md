...
**최신 다음 작업(2026-09-24, D52):** [ML 실험 계획](ML-experiments-next-session.md)을 작성했다. 현재 코호트의 예측 성능 개선을 먼저 진행하고 지원 투수 확대 → CV/의도 → 팬 이해/UI·UX → 운영 순으로 바꾼다. 다음 대화에서 논문/공통 지표 사양 고정·기존 결과 재현·첫 데이터 실험을 실행한다. Astra 판단/독립 검토, Sol medium 실행/기록, 재귀 위임 허용으로 운영 방식을 갱신했다. 이번 마무리에 새 학습·평가·하위 에이전트 실행은 없다. 아래 완료 결과와 G1~G7의 미검증 항목은 그대로 보존한다.

**최신 실행(2026-09-24):** `122b421`에서 ABCD 다음 개발 묶음을 실제 완료했다. A 사양 `885bb76`/후보 고정 `23b9de4`, B 151타석/563구 추천 진단 및 두 후보 기각, C 가용성/모델 지원 분리, D 새 UI·별도 확인 서버, 독립 검토·178개 통합 검사까지 반영했다. [최신 통합 인수인계](ABCD-quality-v3.md)에 G1~G7의 구현·품질 검증, 커밋·결과·재현·프로세스·미측정 사용량을 기록했다. 아래 1~15절은 이전 시점의 기록이다.
...
## 14. 사용자 관전 후 추천 구종 불일치 초기 점검

사용자는 UI 정보 과밀과 큰 구종 불일치를 지적했다. 활성 DB의 이미 공개된 공만 읽어 고유 16구 중 첫 추천 일치 4구, 상위 3개 행동에 실제 구종 포함 8구를 확인했다. QA/사용자 편의 표본이며 전체 성능으로 일반화하지 않는다. 실제 선택과 추천의 목적 차이, 배합 이력 미사용, 작은 모형 값 차이의 결정적 순위, 정책 효과 미검증을 확인했다. [진단 기록](recommendation-choice-diagnosis.md)에 코드/표본/근거/미측정 범위를 기록했다. 다음 한 가지는 A·B 추천 품질·순위 안정성의 사전 고정 진단 사양이다. UI 개편/학습/모델 교체를 이번 질의에서 수행하지 않았으며 사용자 확인용 서버는 유지한다.


## 15. 2026-09-24 세션 종료와 다음 세션 실행 계획

사용자 요청으로 이번 세션은 계획/인수인계만 남기고 종료한다. [다음 세션 실행 계획](ABCD-next-session.md)에 A 총괄+B/C/D 직접 위임, 1차 병렬 진단/개편/교체 근거, 최대 2개 후보의 사전 고정 비교, 독립 검토/통합, CV/운영 합류와 보고 조건을 정리했다. 계획 이후 새 하위 에이전트·실험·UI 변경·예약은 시작하지 않았다. 마무리 당시 확인용 서버 94628/워커 94642 실행, health 정상으로 사용자 관전용 유지했다. 다음 시작은 Git/프로세스 확인 후 A·B 진단 사양을 고정하면서 D·C 독립 작업을 여는 것이다. 상세 실행 경계와 기존 데이터/최종 평가 보존은 새 인수인계를 따른다.

## 16. ABCD 다음 개발 묶음 실제 완료

1. **기준/조직:** `122b421`, 원 작업 트리 clean과 사용자 서버/워커 상태를 확인했다. A/B/C/D 각각 `codex/{a-quality-v3,b-quality-v3,c-evidence-v3,d-ui-v3}` worktree를 분리했다. 동시 한도 4, Sol medium 세 개, 재귀 위임 없음. A가 공통 계약과 최종 판정을 관리했고 C를 B의 독립 검토로 재배정했다.
2. **완료/판정:** 결과 전 추천 진단 고정, 563구 전체 지원 행동 순위/가치·집중도·지원·5개 학습 구성원 민감도, 기존 교체 근거 재검증, 새 관전 UI, 최대 두 후보의 사전 고정 비교와 기각까지 완료했다. 실제 구종 일치·예측 지표를 정책 성능으로 해석하지 않았다. 실제 OPE/CV/교체 효과/팬 이해는 미측정이다.
3. **파일/출력:** `configs/EXP-CHOICE-{DIAG-002,CANDIDATES-003}.json`, 진단/후보/C 근거/보존·운영 감사 스크립트, UI와 회귀 검사. repo `results/{CHOICE-DIAG-002,CHOICE-CANDIDATES-003,C-EVIDENCE-003,ABCD-QUALITY-003}`; SSD는 동일 작업 ID 및 `D-UI-003`. 정확한 명령·SHA·한계는 각 README와 최신 통합 인수인계에 있다.
4. **검증:** Python 178개 통과(1.39초), 실제 K/HR·PC/390px·동일 공·추천 불변·메모/새로고침·이닝/오류·지연 회귀 통과. D 빌드 소스와 통합 웹 소스 일치. 사용자 DB 22세션/56추천/12사건/8메모와 전체 7테이블 내용 불변. 동결 번들/기존 소규모 자료 해시 통과. 실영상/CV 품질 검증으로 집계하지 않는다.
5. **전달/의존:** UI는 8767에서 확인 가능하고 팬 과제를 준비했다. C는 당시 가용성 미확인/모델 미지원/시점 근거 부족을 분리했으며 동일 상태 비교 쌍 0이다. 실제 CV/검토 라벨, 가용성 근거, 팬 참가, 배포/경기 공급 조건만 외부 의존으로 남긴다. 새 정책 평가/지원 확대 연구는 별도 사전 설계를 필요로 한다.
6. **프로세스/사용량:** 기존 8766 PID94628/94642, 새 8767 PID6713/6715를 유지했다. 새 서버는 PPID1로 분리 실행, 재시작 시 저장 세션 복원 확인. 무거운 평가 잔류 없음. 실제 하위 모델은 전부 `gpt-6-sol/medium`, high 없음. 주 모델 정확한 ID·토큰·비용은 미노출/null. 로컬 저장 읽기 동시4·40요청 p95 19.12ms는 실제 계측이며 실시간 서비스 지연은 아니다.

최종 통합 문서 커밋은 `git log -1 --format=%H -- docs/handoffs/A.md`로 확인한다. 인수인계 후 바로 볼 대상은 새 UI이며, 연구의 다음 한 가지는 기존 DEV 재튜닝이 아닌 지원 확대/독립 평가를 위한 새로운 고정 비교 설계다.

## 17. ML 우선순위와 다음 세션 계획

기준 `ec84c89`, 코드·모델 변경 없이 논문 지표/기존 실험·장비를 확인하고 ML0~ML5 계획을 기록했다. SmartPitch의 cross-entropy/Brier, Melville의 보정/시퀀스 절제, Takamido의 이진 분류 지표를 원문과 대조했다. 논문별 입력·정답 차이를 보존하고 현재 공 실제 물리가 투구 전 평가로 새지 않도록 했다. 기존 2024/2025는 노출된 DEV이며 새 최종셋으로 부르지 않는다.

확인 장비는 M4 24 GiB, PyTorch 2.14.0 MPS available, CUDA 없음. 기존 평균 fit 22.7/158.3초는 전처리·400표본 적분 시간을 제외하므로 다음 ML0에서 전 단계 profile을 수행한다. 지금 외부 GPU는 필수가 아니며 큰 작업의 실제 한계가 확인되면 구체안을 만든다.

8766/8767 health 모두 정상, 기존 PID 유지. 원자료·동결 모델·runtime_src·사용자 DB 변경 없음. 문서 링크/차이 검사 후 계획을 커밋한다. 코드 변경이 없어 전체 학습·제품 회귀 검사는 재실행하지 않았다. 이번 계획 턴의 주 모델 정확한 ID/토큰/비용은 도구에서 확인되지 않으며 새 하위 모델 사용은 없다. 최종 커밋은 이 문서의 Git 이력으로 확인한다.
