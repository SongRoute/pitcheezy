...
검증: `npm run build` 통과; `PYTHONPATH=.:apps/observer/backend /Users/song/Projects/pitcheezy/.venv-observer-standalone/bin/python -m pytest apps/observer/backend/tests/test_observer_service.py -q` 14 통과. 테스트는 사전 비공개·동일 공 저장 추천·CAS·재시작·수동 메모/워커 격리, 사건 버전 보존과 실패시 null 수치를 확인한다. 루트 통합 체크아웃의 브라우저 실제 시각 검증은 A가 수행한다.
