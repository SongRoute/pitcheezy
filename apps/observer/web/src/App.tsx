...
function RecommendationCards({ recommendation, comparing, complete, busy, onAdvance }: { recommendation: Recommendation | null; comparing: boolean; complete: boolean; busy: boolean; onAdvance: () => void }) {
  ...
  return <section className="recommendations" aria-labelledby="recommendation-title">
    ...
    {!ready ? <div className="empty-state compact"><span className="empty-symbol">—</span><h3>추천을 제공할 수 없어요</h3><p>{recommendation?.reason || '이 공에 저장된 추천이 없습니다.'}</p><p>추천이 없어도 실제 기록은 계속 확인할 수 있어요.</p></div> : <>
      <div className="lead-choice"><span>추천 구종과 목표</span><strong>{candidates[0].pitch_label || candidates[0].pitch_type}<small>{candidates[0].pitch_type}</small></strong><b>{candidates[0].zone_label}</b><p>포수 시점 · 기록된 도착 위치로 근사한 목표이며 실제 투구 의도는 확인되지 않았어요.</p></div>
      {!complete && <button className="button primary stage-advance" disabled={busy} onClick={onAdvance}>{busy ? <><span className="spinner light" />공을 확인하는 중…</> : <>다음 실제 공 확인<span aria-hidden="true">→</span></>}</button>}
      ...
      <p className="recommendation-note">모델의 선택 제안입니다. 순위 차이가 작아도 실제 경기에서 더 낫다는 뜻은 아니에요.</p>
    ...
    {!ready && !complete && <button className="button primary stage-advance" disabled={busy} onClick={onAdvance}>{busy ? <><span className="spinner light" />공을 확인하는 중…</> : <>다음 실제 공 확인<span aria-hidden="true">→</span></>}</button>}
  </section>;
}
...
function EventCard({ event, pitchNumber }: { event: EventAnalysis | null | undefined; pitchNumber?: number }) {
  ...
  return <section className="event-card" aria-labelledby="event-title"><div className="section-heading"><div><span className="eyebrow">03 · 사건 설명 · 수비팀 승률 모델</span><h2 id="event-title">{pitchNumber ? `타석 마지막 ${pitchNumber}구의 변화` : '이번 타석의 변화'}</h2></div><span className={`event-status ${event.status}`}>{statusLabel}</span></div>
    {developmentOnly ? <p className="event-empty">개발용 합성 사례입니다. 실제 경기의 기여 수치로 표시하지 않습니다.</p> : <>
      ...
      <p className="event-plain">관측: 실제 타석 결과. 추정: 고정 모델의 승률 변화. 미측정: 투구 의도와 선수별 기여.</p>
      <details className="event-technical"><summary>계산 범위와 항목 자세히 보기<span aria-hidden="true">＋</span></summary>
...
export default function App() {
  ...
  return <><header className="site-header"><a className="brand" href="/" aria-label="피치이지 처음 화면"><span className="brand-icon"><BrandMark /></span><span>pitcheezy<span className="brand-dot">.</span></span></a><div className="header-right"><a className="header-lab-link" href="/inning-decisions">교체 직전 기록</a><a className="header-lab-link" href="/video-lab" title="실험용 영상 검토실 · 경기 결과가 포함될 수 있습니다">영상 검토실<small>실험용</small></a><span className="replay-tag"><i />기록으로 보는 야구</span><span className="header-caption">한 공 더 깊게</span></div></header>
    <main className={view ? "observing" : "welcome-mode"}><section className="intro"><div><span className="eyebrow">한 공씩 읽는 야구</span><h1>다음 한 공을,<br className="mobile-break" /> 함께 생각하다.</h1><p>기록을 한 공씩 열어 보며, 그 순간의 선택을 비교해 보세요.</p></div><div className="intro-number" aria-hidden="true">01<span>한 공의 가능성</span></div></section>
      ...
      {loading && !view ? <div className="initial-loading" role="status"><span className="spinner" /><h2>관전 노트를 준비하고 있어요</h2><p>공개된 경기와 저장한 타석을 확인합니다.</p></div> : !view ? <section className="welcome empty-state"><div className="welcome-diamond" aria-hidden="true"><span /></div><span className="eyebrow">아직 열지 않은 한 공</span><h2>{catalog?.games.length ? '타석을 선택하면, 이야기가 시작됩니다.' : '관전할 경기 기록이 아직 없어요.'}</h2><p>{catalog?.games.length ? '추천을 먼저 살펴보고 실제 공을 한 개씩 확인하세요. 결과는 직접 열어 보기 전까지 표시되지 않습니다.' : '경기 목록이 준비되면 위에서 타석을 선택할 수 있습니다.'}</p><div className="welcome-steps"><span>01 · 상황 읽기</span><span>02 · 추천 살펴보기</span><span>03 · 실제 공과 비교</span></div></section> : <>
        ...
        <div className="decision-flow" aria-label="한 공의 관전 순서"><span className={!comparing ? 'current' : ''}>1 · 다음 추천</span><span className={comparing ? 'current' : ''}>2 · 같은 공 실제 비교</span><span className={view.complete ? 'current' : ''}>3 · 사건 설명</span></div>
        <div className="recommendation-stage"><div className="stage-timing"><span className="stage-number">01</span><div><h2>{comparing ? `${selectedPitch!.pitch_number}구를 던지기 전의 저장 추천` : '아직 던지지 않은 다음 공의 추천'}</h2><p>{view.plate_appearance.pitcher_label} 투수 → {view.plate_appearance.batter_label} 타자 · {comparing ? `선택한 과거 ${selectedPitch!.pitch_number}구, 투구 전 ${selectedPitch!.pre_state.balls}볼 ${selectedPitch!.pre_state.strikes}스트라이크에 저장` : '실제 결과를 보기 전에 저장된 모델 제안'}</p></div></div><div className="chart-tabs" role="group" aria-label="투구 비교 시점"><button aria-pressed={!comparing} disabled={view.complete} className={!comparing ? 'active' : ''} onClick={() => { setChartMode('next'); setSelectedPitchId(null); }}>다음 공 추천</button><button aria-pressed={comparing} disabled={!view.last_pitch} className={comparing ? 'active' : ''} onClick={() => { setChartMode('actual'); setSelectedPitchId(null); }}>{comparing && !isLastSelected ? '선택한 공 비교' : '방금 던진 공'}</button></div><RecommendationCards recommendation={recommendation} comparing={comparing} complete={view.complete} busy={busy} onAdvance={() => void mutate('advance')} /></div>
        {comparing && <section className="comparison-stage" aria-labelledby="comparison-title"><div className="stage-timing"><span className="stage-number">02</span><div><h2 id="comparison-title">같은 {selectedPitch!.pitch_number}구, 실제 기록과 비교</h2><p>사전 추천은 이후 결과로 바뀌지 않습니다. 실제 구종과 도착 위치는 기록된 사실입니다.</p></div></div><div className="same-pitch-comparison" aria-label="이 공의 사전 추천과 실제 기록 비교"><div><span>던지기 전 저장 추천 · 모델 추정</span><strong>{savedTop ? `${savedTop.pitch_label || savedTop.pitch_type} · ${savedTop.zone_label}` : '추천 정보 없음'}</strong></div><span aria-hidden="true">→</span><div><span>실제 {selectedPitch!.pitch_number}구 · 관측 기록</span><strong>{selectedPitch!.pitch_label || selectedPitch!.pitch_type || '구종 정보 없음'} · {actualZoneLabel(selectedPitch!)}</strong><small>{selectedPitch!.result_label}</small></div></div></section>}
        <details className="observation-details"><summary>구역 그림과 투수 기록 자세히 보기<span aria-hidden="true">＋</span></summary><div className="workspace"><section className="zone-panel" aria-labelledby="zone-title"><div className="section-heading"><div><span className="eyebrow">공이 향하는 곳</span><h2 id="zone-title">{comparing ? `${selectedPitch!.pitch_number}구, 추천과 실제` : '다음 공의 추천 구역'}</h2></div><span className="zone-view-tag">포수 시점</span></div>
        ...
        </section><aside className="right-column"><section className="matchup" aria-label="투수와 타자"><span className="eyebrow">마운드와 타석</span><div className="matchup-player"><div className="player-symbol" aria-hidden="true">P</div><div><span>투수</span><h2>{view.plate_appearance.pitcher_label}</h2></div></div><div className="batter-row"><span>상대 타자</span><strong>{view.plate_appearance.batter_label}</strong>{view.plate_appearance.batter_stand && <span className="hand-badge">{view.plate_appearance.batter_stand === 'L' ? '좌타' : view.plate_appearance.batter_stand === 'R' ? '우타' : view.plate_appearance.batter_stand}</span>}</div><PitcherContextSummary context={view.context} notes={view.context_notes || []} /></section><ObservedContext context={view.context} /></aside></div></details>

        {view.complete && <EventCard event={view.event_analysis} pitchNumber={view.summary?.selected_pitch_number} />}
        ...
        {view.complete && <TerminalAnalysis view={view} zones={zones} busy={busy} onManual={(zone) => void mutate('manual', zone)} />}
