...
루트 통합 완료: 실행 코드 `62b423f`, C/B와 함께 backend/계약/평가 검사 150개 및 웹 빌드 통과. 실제 삼진(1440px)·홈런(390px) 브라우저 재생, 사전 추천/사건 불변·수동 메모 격리·새로고침·가로 넘침 없음 확인. 아래 독립 D 브랜치 검증에 이어 완료한 통합 결과·재현/화면 경로는 [ABCD 통합 인수인계](ABCD-integration.md)를 따른다.
...
- 동시 GET이 같은 사건을 계산하더라도 쓰기 transaction에서 최신 결과를 다시 조회해 먼저 저장된 값을 사용한다. 직접적인 서로 다른 결과의 동일 revision 저장은 계속 거부한다.
...
동결 모형으로 `codex/d-ui-v3`의 정보 구조를 개편했다. 새 UI, 독립 preview, 실제 데스크톱·390px 화면과 브라우저 검증, 팬 평가 과제는 [D-UI-003](D-UI-003.md)을 따른다. 기존 라이브 서비스와 세션은 유지했다.
