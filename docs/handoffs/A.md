...
계약 `f821b55`, 코드 `9c6d8f9`. 실제 C 결과를 격리 SQLite에 등록하고 게임/이벤트 시점/현 투수/초기 상태/phase/revision 대조 조회를 구현·검증했다. Python 105개 및 API 응답의 D 표시 소비 검사 통과. 기본 DB·모형·원자료·2026/최종 자료는 유지했다. [C→D API 인수인계](C-D-api-v1.md)에 재현·산출물·로그·사용 역할을 기록했다. 다음 한 가지는 독립 기록 선택과 UI 카드 연결이다. 실제 교체 효과/CV는 미완료다.

## 12. 독립 기록 선택/UI 연결 완료

코드 `62945c7`. `/inning-decisions`에 등록 경기·교체 직전 시점 선택과 이닝 전망 카드를 연결했다. 실제 격리 DB/HTTP API로 PC·모바일을 검증했고 빈 목록·오류/재시도·불일치·지연 응답을 확인했다. backend 38개·클라이언트·build·브라우저 검사 통과. [C→D UI 인수인계](C-D-ui-v1.md)에 실행 명령·스크린샷·모델/자료 보존·Astra/Sol 역할을 기록했다. 기본 DB는 변경하지 않았고 서버는 종료했다. 다음 한 가지는 기존 관전 서버의 모델/데이터/저장 세션과 함께 전체 실행 흐름 리허설이다. 실제 교체 효과/CV는 여전히 미완료다.


## 13. 전체 실행 리허설과 사용자 원격 확인 완료

검증 코드 `a8cd5d5`. 실제 동결 모델·기존 3개 저장 세션/6개 추천/2개 사건 결과와 이닝 결정을 별도 실행 공간에 함께 구성했다. 기존 기록 보존, 삼진/홈런 재생·새로고침·타석 이동·이닝 페이지 왕복·PC/모바일 및 catalog 검사를 통과했다. 사용자 맥북에서 SSH/Tailscale 터널 접속도 확인했다. [전체 리허설 인수인계](C-D-full-rehearsal.md)에 버전/데이터 해시/재현/실패 수정/검증/역할을 기록했다. 서버 PID 94628·워커 94642는 사용자가 확인할 수 있도록 유지하며 로그는 SSD `C-D-FULL-001`에 있다. 다음 한 가지는 사용자의 관전 피드백에 따른 D 사용성 수정 범위 확정이다. 새 학습·실제 교체 효과·CV 완료를 의미하지 않는다.
