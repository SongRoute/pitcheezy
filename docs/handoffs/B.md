...
- `PYTHONPATH=.:apps/observer/backend .venv-observer-standalone/bin/python -m pytest tests/test_b_recommendation.py -q`: 합성 실행 분포 입출력, 경계 소스 변경 시 캐시 identity 변경, 같은 identity 문자열의 임의 provider 거부를 포함한다. 별도 standalone 실모델 smoke도 ready/39 후보/확률합/공개 추천 일치를 확인했다.
