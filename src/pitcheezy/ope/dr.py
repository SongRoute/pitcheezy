...
def _sorted_view(df: pd.DataFrame, state_id: np.ndarray, *arrays: np.ndarray):
    ...
    if len(d) == 0:
        return d, [state_id[o].astype(np.int64)] + [a[o] for a in arrays], np.zeros(0, np.int64), gk, np.zeros(0, np.int64)
...
def onestep_dr_terms(df: pd.DataFrame, state_id: np.ndarray, rho: np.ndarray, has: np.ndarray, q_b: np.ndarray, v_e: np.ndarray) -> pd.DataFrame:
    """타석별 1스텝 DR 조각 (game_pk, at_bat_number, sum_rho, sum_rho_q, sum_v, n_dec, n_zero). rho·has 는 df 위치 정렬 (ips.pitch_ratios).
    ...
    return pd.DataFrame({
        ...
        "n_zero": np.bincount(inv, weights=(hd & (rho_ == 0)).astype(float), minlength=n),
    })
