...
남은 것은 실제 CV의 동일 투구·릴리스 이전·좌표 검증과 제구/기여 품질, 당시 교체 후보 명단 및 별도 이닝 평가기, 식별 가능한 정책 효과와 최종 확인이다. **다음 한 가지: 실제 CV 담당자 산출물 1건을 기존 IntentEstimate 인수 조건으로 검사하고, 자료가 없으면 unavailable을 유지한다.** S2 학습 확대는 자동 진행하지 않는다.
