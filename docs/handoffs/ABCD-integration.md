...
**다음 한 가지: 실제 CV 담당자의 동일 투구 산출물 1건을 `IntentEstimate`의 투구 ID·좌표·릴리스 이전 시점·검토 상태로 인수 검사한다.** 실제 입력이 없으면 unavailable을 유지하고 성능 수치를 만들지 않는다. 새 B 후보나 S2 확장은 이번 악화 원인과 별도 고정 실험 사양을 검토한 뒤 진행한다.
