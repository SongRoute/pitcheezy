...
- 상태 `state_id` = ((count_id × 24 + base_out_id) × K + cluster_id) × C + context_id, S = 288 × K × C
...
- v1.2 (2026-09-20, D37): 스키마·저장 포맷 변경 없음. 생산자(`count.fit(out_dir=)`)가 `P.npy`·`n_obs.npy` 를 memmap 으로 직접 쓸 수 있고 `TransitionTensor.save` 는 그 경우 두 파일을 다시 쓰지 않는다(값·해시 동일). 큰 텐서(K=6×C=7, P 26GB)는 `load(mmap=True)` 로만 읽는다
